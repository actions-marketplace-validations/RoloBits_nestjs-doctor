...
| `0` | Passed |
| `1` | A gate failed |
| `2` | Invalid input (bad path, invalid flags, or no TypeScript files under the path) |
...
Both gates run after output, so a `1` still carries the full report. A `2` means no scan ran, and it emits no payload.
