...
A finished `--init` or `ci install` reports one `command_completed` event with two fields: `command`, either `init` or `ci_install`, and `from`, either `flag` or `menu`. It carries the same `distinct_id` as a scan, which under a recognized CI provider is the per-repository hash described under [Identity](#identity), and nothing else about the project. The `--init` flag runs no scan, so only the root config's `telemetry: false` is read; from the menu a sub-project opt-out stops it too.
...
| `trigger` | How the run started: `action`, `ci`, `hook`, `skill` (an agent started the run and has the nestjs-doctor skill installed), `agent`, `script`, `npx` or `global` |
...
`NESTJS_DOCTOR_TRIGGER` overrides the detected `trigger` when set to one of the eight values above. Any other value is ignored. `skill` is `agent` plus an installed skill: the CLI looks for the skill file under the detected agent's config directory in the home the environment names, and reads nothing else.
...
| Control | Effect |
|---|---|
| `--no-telemetry` | Scans and writes the report, sends nothing; `--init` and `ci install` send nothing either |
